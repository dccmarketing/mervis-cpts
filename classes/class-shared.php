...
class Mervis_CPTS_Shared {
	...
	private function get_cache_name( $args, $cache = '' ) {
		...
		$return = MERVIS_CPTS_SLUG . '_plugin_name';

		if ( ! empty( $cache ) ) {

			$return = MERVIS_CPTS_SLUG . $cache . '_plugin_name';

		}

		if ( ! empty( $args['taxonomyname'] ) ) {

			$return = MERVIS_CPTS_SLUG . $cache . $args['taxonomyname'] . '_plugin_name';
	...
	public function query( $params = array(), $cache = '' ) {
		...
		$return 	= wp_cache_get( $cache_name, MERVIS_CPTS_SLUG . '_posts' );

		if ( false === $return ) {

			$args = apply_filters( MERVIS_CPTS_SLUG . '-query-args', $this->set_args( $params ) );
			$query 	= new WP_Query( $args );

			if ( is_wp_error( $query ) && empty( $query ) ) {

				$options 	= get_option( MERVIS_CPTS_SLUG . '-options' );
				$return 	= $options['none-found-message'];

			} else {

				wp_cache_set( $cache_name, $query, MERVIS_CPTS_SLUG . '_posts', 5 * MINUTE_IN_SECONDS );
